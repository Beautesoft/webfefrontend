      ...
      <div className="maintable table-container">
        <div className={`maintable-content ${overFlow ? 'table-responsive' : ''} ${className}`}>
          <table className="table table-striped rounded">
            <thead>
              <tr>
                {headerDetails.map(({ label, className, divClass = '', sortKey = '', element, width = '' }, index) => {
                  return (
                    <th className={className} key={index} style={{minWidth: width ? width:(label.length*10) + 50 +'px'}}>
                      <div
                        className={`d-flex align-items-center justify-content-center text-center ${sortKey &&
                      ...
                      >
                        ...
                        {sortKey ? (
                          <div
                            className={`d-flex table-filter ml-2 mb-1 ${currentSortKey === sortKey &&
                              'active-filter'}`}
                          >
                            <span
                              className={`icon-sort-up fs-14 ${currentSortKey === sortKey && orderBy === 'asc' && 'active'}`}
                            />
                            <span
                              className={`icon-sort-down fs-14 ${currentSortKey === sortKey &&
                                orderBy === 'desc' &&
                                'active'}`}
                            />
                          </div>
        ...
        {pageMeta && <Pagination handlePagination={this.handlePagination} pageMeta={pageMeta} />}
      </div>
    );
  }
}
