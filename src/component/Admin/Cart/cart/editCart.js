...
export class EditCartClass extends Component {
  ...
  getStafflist = (data) => {
    let { page } = this.state;
    this.props
      .getCommonApi(`empcartlist/?sales_staff=1&page=${page}`)
      .then(async (key) => {
        let { status, data } = key;
  ...
  quantityChange = () => {
    ...
    let total_disc =
      cartDetail.discount_amt + cartDetail.additional_discountamt;

    formFields["discount_price"] = Number(
      formFields.price - total_disc
    ).toFixed(2);

    let after_linedisc =
      (formFields.price - cartDetail.discount_amt) * formFields.quantity;

    formFields["trans_amt"] = Number(
      after_linedisc - cartDetail.additional_discountamt
    ).toFixed(2);

    formFields["deposit"] = Number(
      after_linedisc - cartDetail.additional_discountamt
    ).toFixed(2);
  ...
  handleAddstaff = (data) => {
    ...
    formFields["sales_staff"] = formFields["sales_staff"].reduce(function (
      a,
      b
    ) {
  ...
  handleUpdateCart = () => {
    ...
    if (this.validator.allValid()) {
      this.props
        .commonUpdateApi(
          `itemcart/${this.props.id}/?sales_all=0&deposit=${formFields.deposit}`,
          formFields
        )
        .then((key) => {
          let { status, data } = key;
    ...
    return (
      <div className="row new-cart treatment-done">
        <div className="col-12">
          <p className="fs-18 font-700 mb-3 title">{t("Edit Cart")}</p>
        </div>
        ...
        <div className="col-8 mb-2">
          ...
          <div className=" d-flex mb-2">
            {formFields.sales_staff.length > 0
              ? formFields.sales_staff.map((item, index) => {
                  return (
                    <div className="mx-1" key={index}>
                      <span className="">{this.selectedStaff(item)}</span>
                      <img
                        onClick={() => this.removeSalesStaff(index)}
          ...
          {cartDetail.id ? (
            <Discount
              ...
              id={this.props.id}
              handleRefresh={this.getCartData}
            ></Discount>
          ) : (
            ""
          )}
...
export const EditCart = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(EditCartClass)
);
