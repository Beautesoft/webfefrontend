...
export class TreatmentDoneClass extends Component {
    ...
    componentWillMount = () => {
        ...
        this.props.getCommonApi(`empcartlist/?sales_staff=0`).then((key) => {
            let { status, data } = key;
            if (status === 200) {
    ...
    render() {
        let { staffList = [], tstaffList = [], roomList, sourceList, formFields, headerDetails, cartData, postFields, updateFields, duration } = this.state;
                ...
                <div className="col-12">
                    <div className="staff-listing d-flex emp-list">
                        ...
                        {staffList.length > 0 ? staffList.map((item, index) => {
                            ...
                            return (
                                <div className="mx-1 cursor-pointer" key={index} onClick={() => this.handleAddstaff(item)}>
                                ...
                                {tstaffList.length > 0 ? tstaffList.map((item, index) => {
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td className="position-relative status-type"><span className={``}></span><div className="d-flex align-items-center justify-content-center">{item.helper_name}</div></td>
                                            <td><div className="d-flex align-items-center justify-content-center">{item.wp1}</div></td>
                                            <td>
