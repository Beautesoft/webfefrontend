...
export class TopupPrepaidClass extends Component {
    ...
    getCart = () => {
        let { roomList, sourceList, staffList, cartData, duration, treatmentList } = this.state;
        this.props.getCommonApi(`topupprepaid/?cust_id=${this.props.id}`).then((key) => {
            // let { status, data } = key;
            // if (status === 200) {
            cartData = key;
                                ...
                                {treatmentList.length > 0 ? treatmentList.map((item, index) => {
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td><div className="d-flex align-items-left justify-content-left">{item.pp_desc}</div></td>
                                            ...
                                            <td><div className="d-flex align-items-center justify-content-center">{item.remain}</div></td>
                                            ...
                                            <td><div className="d-flex align-items-center justify-content-center">{item.staff_name}</div></td>
                                        </tr>
                                    )
                                }) : ""
                                }
...
export const TopupPrepaid = connect(mapStateToProps, mapDispatchToProps)(TopupPrepaidClass)