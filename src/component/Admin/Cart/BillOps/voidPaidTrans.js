...
export class VoidPaidTransClass extends Component {
    ...
    getCart = (data) => {
        ...
        this.props.getCommonApi(`void/?from_date=${dateFormat(fromDate)}&to_date=${dateFormat(toDate)}&transac_no=${transCode}&cust_code=${basicApptDetail.custId}&cust_name=${custName}&page=${page}&limit=${limit}`).then((res) => {
    ...
    render() {
        let { tstaffList = [], headerDetails, formFields, isOpenvoidDetail, detailDataList, detailHeaderDetails, isOpenvoidCheckout, reason, reasonOption, isvoidCompleted } = this.state;
        let { custName, fromDate, toDate, transCode, custCode } = formFields;
        return (
            <div className="row new-cart treatment-done top-up">
                        ...
                        <div className="table">
                            ...
                            >
                                {tstaffList.dataList && tstaffList.dataList.length > 0 ? tstaffList.dataList.map((item, index) => {
                                    return (
                                        <tr key={index} onClick={() => this.handleSelectVoid(item, index)}>

                                            <td><div className="d-flex align-items-center justify-content-center">{item.sa_transacno_ref}</div></td>
                                            <td><div className="d-flex align-items-center justify-content-center">{item.sa_custno}</div></td>
                                            <td><div className="d-flex align-items-center justify-content-center">{item.sa_custname}</div></td>
                                            <td><div className="d-flex align-items-center justify-content-center">{item.sa_date}</div></td>
                                            <td><div className="d-flex align-items-center justify-content-center">{item.sa_status}</div></td>
                                            <td><div className="d-flex align-items-center justify-content-center">{item.sa_transacno_ref}</div></td>
                                            <td><div className="d-flex align-items-center justify-content-center">{item.payment_remarks}</div></td>
                                        </tr>
                                    )
                                }) : ""
                                }
                ...
                <NormalModal className={"transaction-done-modal top-up-modal"} style={{ minWidth: "560px" }} modal={isOpenvoidDetail} handleModal={this.handleDialog}>
                    ...
                    <div className="mt-3">
                        ...
                        >
                            {detailDataList && detailDataList.length > 0 ? detailDataList.map((item, index) => {
                                return (
                                    <tr key={index} onClick={() => this.handleSelectVoid(item, index)}>
                                        <td><div className="d-flex align-items-center justify-content-center">{index + 1}</div></td>
                                        <td><div className="d-flex align-items-center justify-content-center">{item.desc}</div></td>
                                        <td><div className="d-flex align-items-center justify-content-center">{item.dt_qty}</div></td>
                                        <td><div className="d-flex align-items-center justify-content-center">{item.dt_amt}</div></td>
                                    </tr>
                                )
                            }) : ""
                            }
                ...
                <NormalModal className={"transaction-done-modal top-up-modal"} style={{ minWidth: "560px" }} modal={isOpenvoidCheckout} handleModal={() => { }}>
                    {/* <img onClick={this.handleDialog} className="close cursor-pointer" src={closeIcon} alt="" /> */}
                ...
                <NormalModal className={"void-checkout-complete top-up-modal"} style={{ minWidth: "560px" }} modal={isvoidCompleted} handleModal={() => this.handleDialog}>
                    <img onClick={this.handleDialog} className="close cursor-pointer" src={closeIcon} alt="" />
                    <p className="fs-18 f-600">Void Checkout</p>
...
export const VoidPaidTrans = connect(mapStateToProps, mapDispatchToProps)(VoidPaidTransClass)