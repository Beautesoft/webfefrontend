...
export class AllServicesClass extends Component {
  ...
  queryHandler = (data) => {
    ...
    this.props
      .getServices(`?page=${page}&limit=${limit}&search=${search}`)
      .then((res) => {
        // console.log(res, "dsfdfaafg", res.data.dataList[0].images)
        this.setState({
          servicesList: res.data.dataList,
              ...
              <div key={index} className="services">
                <div className="service1">
                  <div className="listImage mr-4">
                    ...
                    <img
                      className=""
                      src={data.images ? data.images : Service1}
                      width="128"
                      alt=""
                    />
                  </div>
                    ...
                    {currentIndex === index ? (
                      <>
                        ...
                        <div className="option card">
                          <Link to={`/admin/service/${data.id}/serviceDetails`}>
                            <div className="d-flex align-items-center fs-16 pt-3">
                              <span className="icon-eye-grey px-3"></span> View{" "}
                            </div>
                          </Link>
                          <div className="d-flex align-items-center fs-16">
                            <Link to={`/admin/service/${data.id}/editServices`}>
                              <span className="icon-edit px-3"></span> Edit{" "}
                            </Link>
                          </div>
                          <div
                            className="d-flex align-items-center fs-16 pb-3"
                            onClick={() => this.handleDeleteServices(data.id)}
                          >
                            <span className="icon-delete px-3"></span> Delete{" "}
                          </div>
                        </div>
                      </>
                    ) : (
...
export const AllServices = connect(
  mapStateToProps,
  mapDispatchToProps
)(AllServicesClass);
