...
export class CreateServiceClass extends Component {
    ...
    this.validator = new SimpleReactValidator({
      validators: {
        ...
        number: {
          ...
          rule: (val, params, validator) => {
            return (
              validator.helpers.testRegex(val, /^[0-9\b]+$/) &&
              params.indexOf(val) === -1
            );
          },
          messageReplace: (message, params) =>
  ...
  handleSubmit = () => {
    if (this.validator.allValid()) {
      ...
      if (this.props.match.params.id) {
        this.props
          .updateServices(`${this.props.match.params.id}/`, formData)
          .then((res) => {
            console.log(res);
            if (res.status === 201) {
  ...
  render() {
    ...
    let { Course, category, Price, service_tax, discount, image } = formFields;
    let { t } = this.props;
          ...
          <div className="form-group mb-4 pb-2">
            ...
            <div className="col-md-12 p-0">
              ...
              {!this.props.match.params.id
                ? this.validator.message("image", image, "required")
                : ""}
            </div>
          </div>
...
export const CreateService = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(CreateServiceClass)
);
