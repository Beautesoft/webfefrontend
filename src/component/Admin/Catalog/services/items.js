...
export class ServicesItemClass extends Component {
    ...
    handleDialog = () => {
        let { isOpenEditDetail, selectedId, isOpenPriceModal, isOpenCustomer, selectedData, selectedPrice, uomId, is_foc } = this.state;
        isOpenEditDetail = false;
        selectedId = "";
        isOpenPriceModal = "";
        selectedData = {};
    ...
    handleAddCart = async (data, price, id) => {
        // let obj = data;
        let { selectedData, isOpenCustomer, selectedPrice, uomId, is_foc } = this.state
        selectedData = data
        selectedPrice = price
        uomId = id
        isOpenCustomer = true;
    ...
    handleCreateCart = (data, price, id) => {
        let { formFields, selectedData, selectedPrice, uomId, is_foc } = this.state;
        let payload = []
        let obj = {
    ...
    render() {
        let { productCard = {}, isOpenEditDetail, selectedId, list, isOpenPriceModal, serviceName, serviceDetail, search, isOpenCustomer, customerOption, selectedData, selectedPrice, uom } = this.state;
        ...
        return (
            <>
                        ...
                        {list.length > 0 && list.map((data, index) => (
                            <div className={`product-card card `} key={index}>
                                ...
                                <div className="d-flex justify-content-between px-3 card-title">
                                    <p className="label" onClick={this.props.id === "RETAIL" ? () => this.handleSelectPrice(data, index) : () => this.handleAddCart(data)}>{data.item_desc}</p>
                                    <div className="cart-img">
                                        {/* <img src={CartImg} alt="" onClick={this.props.id==="RETAIL" ? ()=>this.handleSelectPrice(data, index):()=>this.handleAddCart(data)}/> */}
                                        <span className="foc-icon" onClick={this.props.id === "RETAIL" ? () => this.handleSelectPrice(data, index) : () => this.handleAddFOC(data)}>
                                ...
                                {this.props.id === "RETAIL" ? <p onClick={() => this.handleSelectPrice(data, index)} className="cost px-3">$ Select</p> : <p className="cost px-3" onClick={this.props.id === "RETAIL" ? () => this.handleSelectPrice(data, index) : () => this.handleAddCart(data)}>${data.item_price}</p>}

                                <div className="product-img px-1" onClick={this.props.id === "RETAIL" ? () => this.handleSelectPrice(data, index) : () => this.handleAddCart(data)}>
                ...
                <NormalModal className={"multiple-appointment"} style={{ minWidth: "760px" }} modal={isOpenEditDetail} handleModal={this.handleDialog}>
                    ...
                    <ItemDetail id={selectedId} menuId={this.props.menuId} api={this.props.api} handleAddCart={this.props.id === "RETAIL" ? () => this.handleSelectPrice() : () => this.handleAddCart()}></ItemDetail>
                </NormalModal>
