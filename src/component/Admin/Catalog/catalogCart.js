...
export class CatalogCartClass extends Component {
  ...
  getServices = (query) => {
    ...
    if (this.props.id) {
      this.props
        .getCommonApi(`servicestock/?Item_Deptid=${this.props.id}&page=${page}`)
        .then((key) => {
          let { status, data } = key;
          if (status === 200) {
        ...
        <div className="products">
          ...
          <div className="d-flex list flex-wrap justify-content position-relative ">
            {productCard.dataList &&
              productCard.dataList.map((data, index) => (
...
export const CatalogCart = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(CatalogCartClass)
);
