...
export class TreatmentListClass extends Component {
  ...
  componentWillMount() {
    ...
    this.props
      .getBookAppointmentList(`?appt_id=${this.props.id}`)
      .then((res) => {
  ...
  render() {
    ...
    let { customerDetail, bookAppointmentList, t } = this.props;
    ...
    return (
      <div className="select-treatment book-treatment mt-3 w-100">
        <div className="row">
          <div className="col-7">
            <div className="row">
              <div className="col-7 f-600">{t("Treatment")}</div>
              <div className="col-5 f-600 ">{t("Price")}</div>
            </div>
          </div>
          <div className="col-5">
            <div className="row">
              <div className="col-7 f-600 ">{t("Quantity")}</div>
              <div className="col-5 f-600 ">{t("Total Price")}</div>
            </div>
          </div>
        </div>
        {bookAppointmentList.length > 0
          ? bookAppointmentList.map(
              (
                {
                  id,
                  PIC,
                    ...
                    <div className="col-7 h-100">
                      <div className="row">
                        <div className="col-7 ">
                          ...
                          <p className="detail mb-0 fs-10">
                            {treatment_date
                              ? this.getDateTime(treatment_date)
                              : ""}
                            , {site_name}
                          </p>
                        </div>
...
export const TreatmentList = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(TreatmentListClass)
);
