...
export class CartClass extends Component {
  ...
  getListDate = () => {
    ...
    this.props
      .getCommonApi(
        `empcartlist/?Appointment_id=${this.props.id}&sales_staff=1`
      )
      .then((res) => {
    ...
    this.props
      .getCommonApi(
        `empcartlist/?Appointment_id=${this.props.id}&sales_staff=0`
      )
      .then((res) => {
        console.log(res, "fdsgsdfgsdf");
  ...
  getCart = () => {
    ...
    this.props
      .getAppointmentCartList(`?Appointment_id=${this.props.id}`)
      .then((res) => {
  ...
  handlDiscountChange = ({ target: { value, name } }) => {
    ...
    if (name === "deposit") {
      ...
      if (name === "discount") {
        ...
        discountField["discount_amt"] =
          (value / 100) *
          (selectedTreatment.price * selectedTreatment.quantity);
  ...
  handleUpdateTreatment = () => {
    ...
    this.props
      .UpdateCart(
        `${this.state.editTreatmentId}/?sales_all=${
          allSaleStaff ? "1" : "0"
        }&service_all=${allServiceStaff ? "1" : "0"}${
  ...
  discountChange = ({ target: { value, name } }) => {
    ...
    if (name === "additional_discount") {
      ...
      addDiscountField["additional_discountamt"] =
        (value / 100) * Number(allItemCart.subtotal);
  ...
  updateAddDiscount = () => {
    ...
    this.props
      .commonCreateApi(
        `itemcart/SetAdditionalDisc/?Appointment_id=${this.props.id}`,
        data
      )
      .then((res) => {
        let { status } = res;
  ...
  render() {
    ...
    let { appointmentCartList, t } = this.props;
    let {
      subtotal,
      discount,
      voucher,
      ...
      <>
              ...
              <div className="row item-list-detail">
                <div className="col-9 list">
                  <div className="row my-2">
                    <div className="col-3 text-center">{t("Treatment")}</div>
                    <div className="col-2 text-center">{t("Price")}</div>
                    <div className="col-2 text-center">{t("Discount")}</div>
                    {/* <div className="col-1 text-center">Deposit</div> */}
                    <div className="col-2 text-center">{t("Sales Staff")}</div>
                  ...
                  {appointmentCartList.length > 0
                    ? appointmentCartList.map((item, index) => {
                        let {
                          id,
                          PIC,
                          Site_Code,
                        ...
                        return (
                          <div className="row detail m-2" key={index}>
                            <div className="col-3 treatment">
                              <p className="title mb-2 f-600">
                                {itemdesc} ({quantity})
                              </p>
                              <p className="sub-title mb-2 f-600">
                                {item_class}
                              </p>
                              <p className="time">
                                {appt_date ? this.getDateTime(appt_date) : ""},
                                {sitecode_name}
                              </p>
                            </div>
                            <div className="col-2 fs-18 p-4 title text-center f-600">
                              $ {price}
                            </div>
                            <div className="col-2 fs-18 p-4 title text-center f-600">
                              {item.discount}
                ...
                <div className="col-3 billable-amount">
                  <div className="row">
                    ...
                    <div className="col-6">{t("Transaction Amt")}</div>
                    ...
                    <div className="col-12">
                      <div className="confirm-booking">
                        <NormalButton
                          ...
                          onClick={() =>
                            this.setState({ isOpenUpdateDisount: true })
                          }
                        />
                      </div>
                    </div>
                  ...
                  <div className="right-total text-center">
                    <p className="label">{t("Billable Amount")}</p>
                    <p className="amount text-orenge">
                      $ {allItemCart.billable_amount}
                    </p>
                  </div>
                </div>
              </div>
        ...
        >
          ...
          <div className="row m-5">
            <div className="col-8 fs-18 f-600 py-2">{t("Edit Treatment")}</div>
            <div className="col-4 ">
              <div className="input-group"></div>
            </div>
            <div className="col-6 my-3">{t("Treatment")}</div>
            <div className="col-6 my-3">
              {selectedTreatment ? selectedTreatment.itemdesc : ""} (
              {selectedTreatment ? selectedTreatment.item_class : ""})
            </div>
            ...
            <div className="col-6 my-3">{t("Branch")}</div>
            <div className="col-6 my-3">
              {selectedTreatment ? selectedTreatment.Site_Code : ""}
            </div>
            <div className="col-6 my-2">{t("Discount in %")}</div>
            ...
            <div className="col-6 my-2">{t("Discount in Amount")}</div>
            <div className="col-6 my-2">
              {modalOpenType === "edit" ? (
                <div className="input-group">
                  <NormalInput
            ...
            <div className="col-6 my-2">{t("Deposit")}</div>
            <div className="col-6 my-2">
              {modalOpenType === "edit" ? (
                <div className="input-group">
                  <NormalInput
            ...
            <div className="col-6 my-2">{t("Sales Staff")}</div>
            <div className="col-6 my-2">
              {modalOpenType === "edit" ? (
                <div className="input-group">
                  <NormalMultiSelect
                    name="sales_staff"
                    value={
                      discountField.sales_staff.length > 0
                        ? discountField.sales_staff
                        : []
                    }
                    className={`staff-skills-select ${
                      discountField.sales_staff !== "" ? "overflow-y-set" : ""
                    }`}
                    options={salesOption}
                    handleMultiSelect={(e) =>
                      this.handleMultiSelect(e, "sales_staff")
                    }
                  ></NormalMultiSelect>
                </div>
              ) : (
            ...
            <div className="col-6 my-2">{t("Services Staff")}</div>
            <div className="col-6 my-2">
              {modalOpenType === "edit" ? (
                <div className="input-group">
                  <NormalMultiSelect
                    name="service_staff"
                    value={
                      discountField.service_staff.length > 0
                        ? discountField.service_staff
                        : []
                    }
                    className={`staff-skills-select ${
                      discountField.service_staff !== "" ? "overflow-y-set" : ""
                    }`}
                    options={servicesOption}
                    handleMultiSelect={(e) =>
                      this.handleMultiSelect(e, "service_staff")
                    }
                  ></NormalMultiSelect>
                </div>
              ) : (
            ...
            <div className="col-6 my-2">{t("Item status")}</div>
            <div className="col-6 my-2">
              {modalOpenType === "edit" ? (
                <div className="input-group">
                  <NormalMultiSelect
                    name="itemstatus"
                    value={
                      discountField.itemstatus.length > 0
                        ? discountField.itemstatus
                        : []
                    }
                    className={`staff-skills-select ${
                      discountField.itemstatus !== "" ? "overflow-y-set" : ""
                    }`}
                    options={itemOption}
                    ismulti={false}
                    handleMultiSelect={(e) =>
                      this.handleSelect(e, "itemstatus")
                    }
                  ></NormalMultiSelect>
                </div>
              ) : (
        ...
        >
          ...
          />
          <div className="row m-5">
            <div className="col-12 fs-22 f-600 p-0 mb-3">{t("Confirm")}</div>

            <div className="fs-16">
              {t("Are you sure want to Delete this treatment from Cart?")}
            </div>
...
export const Cart = withTranslation()(
