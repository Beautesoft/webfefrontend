...
export class TreatmentDetailClass extends Component {
  ...
  componentWillMount() {
    ...
    this.props
      .getCommonApi(`room/?Appointment_id=${this.props.appointmentDetail.id}`)
      .then((res) => {
    ...
    this.props
      .getCommonApi(
        `treatment/Staffs/?Appointment_id=${this.props.appointmentDetail.id}`
      )
      .then((res) => {
  ...
  handleSubmit = async () => {
    ...
    await this.props
      .createBooking(`${this.props.treatmentOutletDetail.id}/`, treatmentDetail)
      .then((res) => {
        if (res.status) {
          let { formFields } = this.state;
          formFields = {
  ...
  render() {
    let { outletOption, staffOption, roomOption } = this.state;
    let { treatmentDetail, treatmentOutletDetail, t } = this.props;
    let { course, item_class, site_name, treatment_date, PIC, price } =
      treatmentOutletDetail;
    let {
      outlet,
      emp_no,
      Trmt_Room_Codeid,
      cus_requests = treatmentOutletDetail.cus_requests,
      treatment_no,
    } = treatmentDetail;
    return (
      <div className="select-treatment w-100">
          ...
          <div className="col-7">
            <div className="row">
              ...
              <div className="col-8 header-detail">
                ...
                <p className="detail">
                  {treatment_date ? this.getDateTime(treatment_date) : ""},{" "}
                  {site_name}
                </p>
              </div>
            </div>
          ...
          <div className="col-5">
            <div className="row">
              <div className="col-6 py-4 header-detail">
                <div>
                  <p className="subtitle">{t("Requests")}</p>
        ...
        <p className="headline">{t("Treatment No")}</p>
        ...
        <p className="headline">{t("Staffs")}</p>
            ...
            <div className="input-select">
              <NormalMultiSelect
                ...
                className={`staff-skills-select ${
                  emp_no.length > 0 ? "overflow-y-set" : ""
                }`}
                options={staffOption}
        ...
        <p className="headline">{t("Rooms")}</p>
        <div className="input-group mb-3 mt-1">
          <div className="input-select">
            <NormalSelect
              // placeholder="Enter here"
        ...
        <p className="headline">{t("Customer Requests")}</p>
        <div className="input-group mb-3 mt-1">
          <NormalTextarea
...
export const TreatmentDetail = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(TreatmentDetailClass)
);
