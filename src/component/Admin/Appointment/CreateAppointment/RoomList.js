...
export class RoomListClass extends Component {
  ...
  handlegetRoomList = async (id = 9) => {
    ...
    this.props
      .getCommonApi(`room/?Appointment_id=${appointmentDetail.id}`)
      .then((res) => {
        this.setState({
          roomList: res.data,
        });
                      ...
                      {roomList.length > 0 &&
                        roomList.map((data, index) => (
                          <div
                            ...
                            onClick={() => this.handleTreatmentDetail(data.id)}
                          >
                            <img
...
export const RoomList = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(RoomListClass)
);
