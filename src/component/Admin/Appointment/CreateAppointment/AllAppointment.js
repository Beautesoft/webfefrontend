...
export class AllAppointmentClass extends Component {
  ...
  componentWillMount = async () => {
    ...
    await this.props
      .getAppointmentList(`?Appt_date=${dateFormat(new Date())}`)
      .then((res) => {});
    this.getListData();
    this.getStaffList(1);
  };
  ...
  render() {
    ...
    let { customerList, t } = this.props;
    return (
      <div className="booked-appointment w-100">
        {confirmedBookedList.length > 0
          ? confirmedBookedList.map(({ Appointment, Treatment }, index) => {
              // await this.getStaffList(Appointment.ItemSite_Codeid)
              return (
                <>
                  <div className="customer-title mb-4">
                    {t("Customer")} {index + 1}*
                  </div>
                  <AppointmentForm
                                ...
                                <div className="col-7">
                                  <div className="row">
                                    ...
                                    <div className="col-8 py-2 header-detail">
                                      <p className="fs-18 title mt-1">
                                        {stock_name}
                                      </p>
                                      <p className="fs-16 subtitle">
                                        {item_class}
                                      </p>
                                      <p className="detail">
                                        {treatment_date
                  ...
                  <div className="confirm mt-3 mb-5">
                    ...
                    <NormalButton
                      ...
                      outline={true}
                      onClick={() =>
                        this.props.handleAnotherTreatment(Appointment)
                      }
                    />
                    <NormalButton
...
export const AllAppointment = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(AllAppointmentClass)
);
