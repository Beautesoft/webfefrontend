...
export class SchedulerClass extends Component {
  ...
  state = {
    ...
    filterType: "day",
    selectedId: "",
    menuOption: [{ lable: "Name", value: "name" }],
    dayWeekMonth: "day",
  };
    ...
    return (
      <div className="row define-height px-3">
        ...
        <div className="col-8 day-week-month d-flex pl-2 ">
          ...
          {/* <div onClick={() => this.handleChangeView('week')} className={`menu ${dayWeekMonth === 'week' ? "active" : ""}`}>{t("Week")}</div>
                    <div onClick={() => this.handleChangeView('month')} className={`menu ${dayWeekMonth === 'month' ? "active" : ""}`}>{t("Month")}</div> */}
        </div>
        <Scheduler
          data={events}
...
export const Calander = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(SchedulerClass)
);
